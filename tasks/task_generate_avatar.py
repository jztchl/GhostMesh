...
def generate_avatar(ai_character_id: str):
    ...
    try:
        ...
        genertatd_image_path = generate_image(
        ...
        compress_image_under_300kb(genertatd_image_path, genertatd_image_path)
        object_name = f"{ai_character_id}+{datetime.now().strftime('%Y%m%d%H%M%S')}.png"
        public_url = upload_file(
            file_path=genertatd_image_path,
        ...
        db.add(ai_character)
        db.commit()
        os.remove(genertatd_image_path)
